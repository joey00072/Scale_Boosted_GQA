...
if len(sys.argv) > 1:
    for idx, arg in enumerate(sys.argv):
        if "--attn_type" in arg:
            attn_type = arg[len("--attn_type="):]
        elif "--num_heads" in arg:
            num_heads = int(arg[len("--num_heads="):])
        elif "--booster" in arg:
            booster = True
        elif "--head_dim" in arg:
            head_dim = int(arg[len("--head_dim="):])    
        elif "--num_kv_heads" in arg:
            num_kv_heads = int(arg[len("--num_kv_heads="):])
            

if num_heads is None:
    num_heads: int = 70 if attn_type=="mla" else (62 if attn_type=="mqa" else 16) # 46 for lora_kv = 128

if head_dim is None:
    head_dim = 32 if attn_type in ["mla", "mqa"] else None
