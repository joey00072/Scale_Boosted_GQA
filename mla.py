...
class Attention(nn.Module):
    def __init__(self, config: Config):
        ...
        self.num_heads = config.num_heads
        self.head_dim = config.head_dim if config.head_dim is not None else config.d_model // config.num_heads
        ...
        if config.booster:
            self.scale = 1/(math.sqrt(self.head_dim ) * self.num_queries_per_kv)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, freqs_cis) -> torch.Tensor:
        ...
        if self.flash_attn:
            ...
            attn_mtx = torch.matmul(q, k.transpose(2, 3)) * self.scale
